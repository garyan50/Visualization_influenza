(function() {
  ...
  document.addEventListener("DOMContentLoaded", function(e) {
    ...
    var width = 100,
      height = 100;
    var maincolor = "#FFDC00";
    var svg = d3.select("body").append("svg");
    var force = d3.layout.force().gravity(.05).distance(50).charge(-100);
    var datanodes = [],
      datalinks = [],
      i = 1;
    var delta = 0;
    ...
    function buildGraph(inlinks, innodes) {
      ...
      node.append("circle").attr("fill", function(d) {
        return d.color;
      }).style("stroke", "#F012BE").style("stroke-width", function(d) {
        return d.page ? 3 : 0;
      }).attr("r", function(d) {
        return d.color == maincolor ? radius * 1.5 : radius;
      }).on("click", function(d) {
        if (d.page) {
          window.open(d.name + ".html", "_self");
        }
      });
      ...
      }).attr("stroke", "#333").attr("dx", 12).attr("dy", ".35em").attr("font-size", "15");
      nodesel.exit().remove();
    ...
    for (var k, k = 1; k < data.length; k++) {
      ...
      if (k > topn) {
        ...
        datanodes.push({
          "name": data[k]["target"],
          color: "#39CCCC",
          page: pages.indexOf(data[k]["target"]) > -1
        });
        order[key] = k;
      }
